    ...
    async def append(self, event: Event) -> bool:
        ...
        def _append():
            ...
            cursor.execute(
                """
                INSERT INTO events 
                (event_id, event_type, aggregate_id, aggregate_type, payload, 
    ...
    def _format_event_details(self, event: Event) -> str:
        ...
        match event.event_type:
            ...
            case EventType.TEXT_EDITED:
                summary = event.payload.get("edit_summary", "changes made")
                return f"Edited text: {summary}"
            case EventType.PANEL_GENERATED:
                model = event.payload.get("model_id", "unknown")
                return f"Generated panel with model {model}"
...
async def log_node_updated(
    node_id: str, changes: dict[str, Any], user_id: str | None = None
) -> None:
