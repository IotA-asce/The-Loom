...
class VectorStore(ABC):
    ...
    def __init__(
        ...
        self.embedding_provider = (
            embedding_provider or EmbeddingProviderFactory.create()
        )
...
class ChromaVectorStore(VectorStore):
    ...
    async def delete_documents(self, document_ids: list[str]) -> int:
        ...
        collection = self._get_collection()
        await asyncio.get_event_loop().run_in_executor(
            None, lambda: collection.delete(ids=document_ids)
        )
    ...
    async def clear(self) -> None:
        ...
        client = self._get_client()
        await asyncio.get_event_loop().run_in_executor(
            None, lambda: client.delete_collection(self.collection_name)
        )
...
class MockVectorStore(VectorStore):
    ...
    async def search(
        ...
        for doc_id, embedding in self._embeddings.items():
            ...
            # Cosine similarity
            dot_product = sum(
                a * b for a, b in zip(query_embedding, embedding, strict=False)
            )
...
async def hybrid_search(
    ...
    k1 = 1.5
    b = 0.75
    avg_doc_len = sum(len(tokenize(doc.text)) for doc in documents) / max(
        1, len(documents)
    )
    ...
    vector_scores: dict[str, float] = {
        result.document.id: result.score for result in vector_results
    }
    ...
    for doc in documents:
        ...
        combined_scores[doc.id] = (
            vector_weight * v_score + keyword_weight * k_score_norm
        )
