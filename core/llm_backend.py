...
The story continues with faithful adherence to the established tone 
and character voices. The scene unfolds with tension that grips the air. 
Characters move through the space with purpose, their actions guided by 
underlying currents of narrative.

"We cannot turn back now," the voice rang out. The response came not 
in speech, but in the firmness of steps forward into uncertainty.
        ...
        async for chunk in stream:
            ...
            if finish_reason:
                yield LLMStreamChunk(
                    content="", is_finished=True, finish_reason=finish_reason
                )
            elif delta.content:
                yield LLMStreamChunk(content=delta.content, is_finished=False)
...
class AnthropicBackend(LLMBackend):
    ...
    async def generate_stream(
        ...
        async with client.messages.stream(**kwargs) as stream:
            ...
            yield LLMStreamChunk(
                content="", is_finished=True, finish_reason=final.stop_reason
            )
...
class GeminiBackend(LLMBackend):
    ...
    def _convert_messages(
        ...
        # (Gemini doesn't support multi-turn via generate_content)
        prompt = "\n\n".join(parts)
...
class OllamaBackend(LLMBackend):
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        return LLMResponse(
            ...
            finish_reason=(
                "stop"
                if not response.get("done_reason")
...
class LLMBackendFactory:
    ...
    def register_backend(
        cls, provider: LLMProvider, backend_class: type[LLMBackend]
    ) -> None:
